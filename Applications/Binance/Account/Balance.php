...
class Balance extends Base
{
    ...
    public function reprice($params)
    {
        ...
        $balance = $this->db()->select('id,currency,reset_order_oid')->from('balance')->where('currency', $currency)->row();
        ...
        $row = $this->db()->select($fields)->from('orders')
                    ->where('symbol', $symbol)->where('oid>', intval($balance['reset_order_oid']))
